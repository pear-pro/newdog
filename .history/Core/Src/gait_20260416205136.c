...
#define jump_freq0 0.0002f   // 0.01
#define jump_freq1 0.0002f   // 0.01
...
float tau = 0.0f;
float t = 0;    
float support_Kp = 0.70f; // 0.6,0.25
float swing_Kp = 0.3f;
...
void motion_Jump(float stride)
{
    ...
    float k0 = (height_des - 15.5f) / (stride / 2.0f); // 斜率
    float x_start0 = 0.0f;
    float x_stop0 = stride / 2.0f;
    ...
    for (float x = stride / 2.0f; x >= -stride / 2.0f; x -= jump_freq1){
        float y = 15.5f;;
        hposition1.B_y = y;
        hposition1.B_x = x; 
        hposition2.B_y = y;
