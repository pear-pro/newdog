...
float motor7_kp_offset = 0.15f;
float motor8_kp_offset = 0.2f;
...
void set_Motor_Kp(int hposition1_state, int hposition2_state, int hposition3_state, int hposition4_state)
{
    ...
    if (hposition1_state == 1) // 支撑相
        {
            hmotor1.Kp = support_Kp + motor1_kp_offset;
            hmotor2.Kp = support_Kp + motor2_kp_offset;
        ...
        {
            hmotor1.Kp = swing_Kp + motor1_kp_offset;
            hmotor2.Kp = swing_Kp + motor2_kp_offset;
    ...
    if (hposition2_state == 1) // 支撑相
        {
            hmotor3.Kp = support_Kp + motor3_kp_offset;
            hmotor4.Kp = support_Kp + motor4_kp_offset;
            hmotor3.Tau_ff = support_tau_ff;
            hmotor4.Tau_ff = support_tau_ff;
        }
    else                      // 摆动相
        {
            hmotor3.Kp = swing_Kp + motor3_kp_offset;
            hmotor4.Kp = swing_Kp + motor4_kp_offset;
    ...
    if (hposition3_state == 1) // 支撑相
        {
            hmotor5.Kp = support_Kp + motor5_kp_offset;
            hmotor6.Kp = support_Kp + motor6_kp_offset;
            hmotor5.Tau_ff = support_tau_ff;
            hmotor6.Tau_ff = support_tau_ff;
        }
    else                      // 摆动相
        {
            hmotor5.Kp = swing_Kp + motor5_kp_offset;
            hmotor6.Kp = swing_Kp + motor6_kp_offset;
    ...
    if (hposition4_state == 1) // 支撑相
        {
            hmotor7.Kp = support_Kp + motor7_kp_offset;
            hmotor8.Kp = support_Kp + motor8_kp_offset;
            hmotor7.Tau_ff = support_tau_ff;
            hmotor8.Tau_ff = support_tau_ff;
        }
    else                      // 摆动相
        {
            hmotor7.Kp = swing_Kp + motor7_kp_offset;
            hmotor8.Kp = swing_Kp + motor8_kp_offset;
...
void motion_Calibrate(float height, float step_height, float stride)
{
    float freq = Forward_freq; // 步频
    tau = tau + freq; // 前进
    if(tau >= 1.0f){tau = 0.0f;}
...
void motion_Mix(float height, float step_height, float stride)
{
    ...
    if (rc_x > 0){
        R_stride = stride - rc_x;
        L_stride = stride;
    }else{
        R_stride = stride;
        L_stride = stride + rc_x;
    }
