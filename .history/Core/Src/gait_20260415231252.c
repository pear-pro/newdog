...
void motion_Mix(float height, float step_height, float stride)
{
    ...
    if(_t >= 1.0f){_t = 0.0f;}
    // 采样点可以采用密疏密使得中间斜率更高
    tau = (cosf (pi*_t-pi)+1.0f)/4.0f+0.5f; // 慢快慢
...
//   }
    ...
    float L_stride = stride;
    if (rcData.R_x > 0){
        R_stride = stride * (1 - 2*rcData.R_x);
        L_stride = stride;
    }else{
        R_stride = stride;
        L_stride = stride * (1 + 2*rcData.R_x);
    }
