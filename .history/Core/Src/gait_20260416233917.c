...
#define jump_freq0 0.006f   // 0.01
#define jump_freq1 0.30f   // 0.01
#define jump_freq2 0.0040f   // 0.40
#define jump_freq3 0.007f   // 0.07
#define jump_freq4 0.001f   // 0.01
...
float support_Kp = 0.20f; // 0.6/0.7,0.25/0.3
float swing_Kp = 0.1f;
float Expect_kw = 0.01;  // 直接用来初始化
float support_tau_ff = 0.00f; // 0.10
float swing_tau_ff = 0.0f;

float kp_GyroZ = 0.13f;  // 150左右输入，20左右输出
...
void motion_Jump(float stride)
{
    ...
    float k0 = (15.5f - walk_height) / (10.0f); // 斜率
    float x_start0 = 0.0f;
    float x_stop0 = 10.0f;
	
//	// (0,walk_height) → (10.0,15.5)
...
//    }
	...
	// (9.0,15.5) → (9.0,-stride / 2.0f)
    for (float i = 0.0f; i <=  1.0f; i += jump_freq1){
		if (emergency_stop==1){return;}
		
        float x = 10.0f + (-stride / 2.0f - 10.0f) * i;
        float y = 15.5f;
        hposition1.B_y = y;
        hposition1.B_x = x; 
        hposition2.B_y = y;
   ...
   for (float i = 1.0f; i >=  0.0f; i -= jump_freq1){
		if (emergency_stop==1){return;}
...
//    for (float x = x_stop0; x >= x_start0; x -= jump_freq0){
//		if (emergency_stop==1){return;}
//        float y = k0 * x + walk_height;
//        hposition1.B_y = y;
//        hposition1.B_x = x; 
