...
void motion_Crawl(float step_height, float stride){
    float Crawl_height = 10.2f; // 电机轴心到地面的距离14.5 - 足底高度4.0
    tau += crawl_freq; // 步频
    if(tau >= 1.0f) { tau -= 1.0f; }

    if (tau <= 0.5f)
    {
        GaitPhasesPoints FrontState = crawl_gaitGenerator(0, Crawl_height, step_height, stride, 0.0f);
        GaitPhasesPoints BackState = crawl_gaitGenerator(0, Crawl_height, step_height, stride, 0.0f);

        hposition1.B_y  = FrontState.ySwing * (1.0f+tanf(pi*stab_roll/180.0f)) ;
        hposition1.B_x  =  FrontState.xSwing; 
        hposition2.B_y  = FrontState.ySupport * (1.0f+tanf(pi*stab_roll/180.0f));
        hposition2.B_x  =  FrontState.xSupport; 
        hposition3.B_y  = BackState.ySwing * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition3.B_x  =  BackState.xSwing; 
        hposition4.B_y  = BackState.ySupport * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition4.B_x  =  BackState.xSupport; 
        set_Motor_Kp(0,1,0,1);
    }
    else if (tau > 0.5f && tau <= 1.0f)
    {
        GaitPhasesPoints FrontState = crawl_gaitGenerator(1, Crawl_height, step_height, stride, 0.0f);
        GaitPhasesPoints BackState = crawl_gaitGenerator(1, Crawl_height, step_height, stride, 0.0f);

        hposition1.B_y  = FrontState.ySupport * (1.0f+tanf(pi*stab_roll/180.0f));
        hposition1.B_x  =  FrontState.xSupport; 
        hposition2.B_y  = FrontState.ySwing * (1.0f+tanf(pi*stab_roll/180.0f));
        hposition2.B_x  =  FrontState.xSwing;
        hposition3.B_y  = BackState.ySupport * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition3.B_x  =  BackState.xSupport;
        hposition4.B_y  = BackState.ySwing * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition4.B_x  =  BackState.xSwing;
        set_Motor_Kp(1,0,1,0);
    }
