 ...
 static void sbus_to_remote_control(volatile const uint8_t *sbus_buffer, SBUS_ctrl_t *sbus_ctrl)
 {
     if((sbus_buffer [0] == 0x0f) && (sbus_buffer[24] == 0x00))//�ж�ͷ֡��β֡
     {
		...
		rcData.R_x_raw = sbus_ctrl -> ch[0]-9;
        rcData.R_y_raw = sbus_ctrl -> ch[1];
		rcData.L_x_raw = sbus_ctrl -> ch[3];
        rcData.L_y_raw = sbus_ctrl -> ch[2];
        rcData.sw5_raw = sbus_ctrl -> ch[4];
        rcData.sw6_raw = sbus_ctrl -> ch[5];
        rcData.sw7_raw = sbus_ctrl -> ch[6];
        rcData.sw8_raw = sbus_ctrl -> ch[7]; 
