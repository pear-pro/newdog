...
void motion_Crawl(float step_height, float stride){
    ...
    float Crawl_height = 10.2f; // y轴方向上电机轴心到地面的距离14.5 - 足底高度4.0
    float front_pivot_x = 12.5f + 0.5f*stride; // 前摆线方程原点=x方向上离电机轴心最近的距离+步幅的一半

    if (tau <= 0.5f)
    {
        GaitPhasesPoints FrontState = crawl_gaitGenerator(0, Crawl_height, step_height, stride, front_pivot_x);
        GaitPhasesPoints BackState = crawl_gaitGenerator(0, Crawl_height, step_height, stride, -front_pivot_x);

        hposition1.B_y  = FrontState.ySwing * (1.0f+tanf(pi*stab_roll/180.0f)) ;
        hposition1.B_x  =  FrontState.xSwing; 
        hposition2.B_y  = BackState.ySupport * (1.0f+tanf(pi*stab_roll/180.0f));
        hposition2.B_x  =  BackState.xSupport; 
        hposition3.B_y  = BackState.ySwing * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition3.B_x  =  BackState.xSwing; 
        hposition4.B_y  = FrontState.ySupport * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition4.B_x  =  FrontState.xSupport; 
        set_Motor_Kp(1,1,1,1);
    }
    else if (tau > 0.5f && tau <= 1.0f)
    {
        GaitPhasesPoints FrontState = crawl_gaitGenerator(1, Crawl_height, step_height, stride, front_pivot_x);
        GaitPhasesPoints BackState = crawl_gaitGenerator(1, Crawl_height, step_height, stride, -front_pivot_x);

        hposition1.B_y  = FrontState.ySupport * (1.0f+tanf(pi*stab_roll/180.0f));
        hposition1.B_x  =  FrontState.xSupport; 
        hposition2.B_y  = BackState.ySwing * (1.0f+tanf(pi*stab_roll/180.0f));
        hposition2.B_x  =  BackState.xSwing;
        hposition3.B_y  = BackState.ySupport * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition3.B_x  =  BackState.xSupport;
        hposition4.B_y  = FrontState.ySwing * (1.0f-tanf(pi*stab_roll/180.0f));
        hposition4.B_x  =  FrontState.xSwing;
        set_Motor_Kp(1,1,1,1);
    }
